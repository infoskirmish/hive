...
def main():
    """ """
    file = open(sys.argv[1]) #in this case, file = project_strings_source.h
    if len(sys.argv) > 2:
    ...
    newheaderfilemain += "void init_strings();"
    newheaderfilemain += "\n#endif\n\n"
    
    forfunc += "}\n"
 
    file = open("proj_strings.h", "w")
    file.write(newheaderfile)
    file.close()
    
    file = open("proj_strings_main.h", "w")
    file.write(newheaderfilemain)
    file.close()
    
    file = open("init_strings.c", "w")
    file.write("#include \"string_utils.h\"\n")
    file.write("#include \"proj_strings.h\"\n")
    file.write(forfunc)
    file.close()
