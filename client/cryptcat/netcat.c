...
//	register char * p;
	...
	if ( dup2 (fd, STDIN_FILENO) < 0 )	/* the precise order of fiddlage */
	{
		err = -1;
		goto error;
	}
	...
	if ( dup2 (STDIN_FILENO, STDOUT_FILENO) < 0 )	/* swiped directly out of "inetd". */
	{
		err = -2;
		goto error;
	}
	...
	// strrchr() returns pointer to last occurrance of char c
...
//		Debug(( " . '/' found. p = %s", p ))
	...
	{
		// p == NULL: char c not found
		p = pr00gie;
//		Debug(( " . '/' not found. p = %s", p ))
	}

//	fprintf( stderr, " . DEBUG => %s:%i\n", __FILE__, __LINE__ );
//	fprintf( stderr, " . DEBUG: executing %s as %s ...\n", pr00gie, p );
//	Debug(( "gonna exec %s as %s...", pr00gie, p ))
//	execlp( pr00gie, p, NULL );
	execl( pr00gie, p, NULL );
//	execl( "/bin/sh", p, "-c", pr00gie, NULL );
...
/* not reached */
	...
	Debug(( " . Leaving doexec()" ))

	return 0;

error:
	dup2( saved_stderr, STDERR_FILENO );
	fprintf( stderr, " . ERROR: err code %i\n", err );
	perror( " goto-error: " );
	return -1;
...
/* Debug (("in go: x now %c, optarg %x optind %d", x, optarg, optind)) */
	...
	o_alla++; break;
      ...
      case 'e':				/* prog to exec */
	pr00gie = optarg;
	Debug(( " . GAPING_SECURITY_HOLE flag set.  Execute %s", pr00gie ));
	break;
#endif
      case 'k':
...
//j: change to code below
Debug (("after go: x now %i, optarg %x optind %d", x, optarg, optind))
...
/* dolisten does its own connect reporting, so we don't holler anything here */
    if (netfd > 0) {
      ...
      if (pr00gie)			/* -e given? */
		{
			Debug(( " . GAPING_SECURITY_HOLE: Executing '%s'", pr00gie ))
			Debug(( " . Executing doexec()" ))
			doexec (netfd);
			Debug(( " . Returning from doexec()" ))
		}
#endif /* GAPING_SECURITY_HOLE */
