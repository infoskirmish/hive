...
// this module only relevant to a Solaris build.
#ifdef SOLARIS 

//#warning Consider removing dlcommon.h, it is only used for Solaris...
 ...
 */
/**************************************/
#define		CASERET(s)	case s:  return ( xs(#s) )
#define		debug_fatal(x, ...)	debug_error(x, __VA_ARGS )
...
void dlinforeq(int fd)
{
	...
	{
		debug_error("dlinforeq:  putmsg() failed.\n");
	}
}
...
void dlinfoack(int fd, char* bufp)
{
	...
	strgetmsg(fd, &ctl, (struct strbuf*)NULL, &flags, xs("dlinfoack") );
	...
	{
		debug_error("dlinfoack:  response ctl.len too short:  %d\n", ctl.len);
		return;
	}

	if (flags != RS_HIPRI)
	{
		debug_error("dlinfoack:  DL_INFO_ACK was not M_PCPROTO\n");
		return;
	}

	if (ctl.len < (int)sizeof (dl_info_ack_t))
	{
		debug_error("dlinfoack:  short response ctl.len:  %d\n", ctl.len);
		return;
	}
}
...
void dlattachreq(int fd, u_long ppa)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlattachreq:  putmsg() failed.\n");
	}
}
...
void dlenabmultireq(int fd, char* addr, int length)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlenabmultireq:  putmsg() failed.\n");
	}
}
...
void dldisabmultireq(int fd, char* addr, int length)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dldisabmultireq:  putmsg() failed.\n");
	}
}
...
void dlpromisconreq(int fd, u_long level)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlpromiscon:  putmsg() failed.\n");
	}

}

void dlpromiscoff( int fd, u_long level)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlpromiscoff:  putmsg() failed.\n");
	}
}
...
void dlphysaddrreq( int fd, u_long addrtype)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlphysaddrreq:  putmsg() failed.\n");
	}
}
...
void dlsetphysaddrreq(int fd, char* addr, int length)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlsetphysaddrreq:  putmsg() failed.\n");
	}
}
...
void dldetachreq( int fd)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dldetachreq:  putmsg() failed.\n");
	}
}
...
void dlbindreq( int fd, u_long sap, u_long max_conind, u_long service_mode, u_long conn_mgmt, u_long xidtest)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlbindreq:  putmsg() failed.\n");
	}
}
...
void dlunitdatareq(int fd, u_char* addrp, int addrlen, u_long minpri, u_long maxpri, u_char* datap, int datalen)
{
	...
	{
		debug_error("dlunitdatareq:  putmsg() failed.\n");
	}
}
...
void dlunbindreq(int fd)
{
	...
	if (putmsg(fd, &ctl, (struct strbuf*) NULL, flags) < 0)
	{
		debug_error("dlunbindreq:  putmsg() failed.\n");
	}
}
...
void dlokack(int fd, char* bufp)
{
	...
	strgetmsg(fd, &ctl, (struct strbuf*)NULL, &flags, xs("dlokack"));
	...
	{
		debug_error("dlokack:  response ctl.len too short:  %d\n", ctl.len);
		return;
	}

	if (flags != RS_HIPRI)
	{
		debug_error("dlokack:  DL_OK_ACK was not M_PCPROTO\n");
		return;
	}

	if (ctl.len < (int)sizeof (dl_ok_ack_t))
	{
		debug_error("dlokack:  short response ctl.len:  %d\n", ctl.len);
		return;
	}
}
...
void dlerrorack(int fd, char* bufp)
{
	...
	strgetmsg(fd, &ctl, (struct strbuf*)NULL, &flags, xs("dlerrorack") );

	dlp = (union DL_primitives *) ctl.buf;

	expecting(DL_ERROR_ACK, dlp);

	if (ctl.len < (int)sizeof (dl_error_ack_t))
	{
		debug_error("dlerrorack:  response ctl.len too short:  %d\n", ctl.len);
		return;
	}

	if (flags != RS_HIPRI)
	{
		debug_error("dlerrorack:  DL_OK_ACK was not M_PCPROTO\n");
		return;
	}

	if (ctl.len < (int)sizeof (dl_error_ack_t))
	{
		debug_error("dlerrorack:  short response ctl.len:  %d\n", ctl.len);
		return;
	}
}
...
void dlbindack( int fd, char* bufp)
{
	...
	strgetmsg(fd, &ctl, (struct strbuf*)NULL, &flags, xs("dlbindack") );
	...
	if (flags != RS_HIPRI)
	{
		debug_error("dlbindack:  DL_OK_ACK was not M_PCPROTO\n");
		return;
	}

	if (ctl.len < (int)sizeof (dl_bind_ack_t))
	{
		debug_error("dlbindack:  short response ctl.len:  %d\n", ctl.len);
		return;
	}
}
...
void dlphysaddrack( int fd, char* bufp)
{
	...
	strgetmsg(fd, &ctl, (struct strbuf*)NULL, &flags, xs("dlphysaddrack") );
	...
	{
		debug_error("dlphysaddrack:  short response ctl.len:  %d\n", ctl.len);
		return;
	}
}
...
void expecting(int prim, union DL_primitives* dlp)
{
	if (dlp->dl_primitive != (u_long)prim) {
		printdlprim(dlp);
		debug_fatal(4, "expected %s got %s\n", dlprim(prim),
...
void strgetmsg(int fd, struct strbuf* ctlp, struct strbuf* datap, int* flagsp, char* caller)
{
	...
	*flagsp = 0;
	...
	{
		debug_error("%s:  MORECTL|MOREDATA\n", caller);
		return;
	}
	if (rc & MORECTL)
	{
		debug_error("%s:  MORECTL\n", caller);
		return;
	}
	if (rc & MOREDATA)
	{
		debug_error("%s:  MOREDATA\n", caller);
		return;
	}
	...
	{
		debug_error("getmsg:  control portion length < sizeof (long):  %d\n", ctlp->len);
		return;
	}
}
...
void printdlinforeq(union DL_primitives* dlp)
{
	...
	//Set dlp to itself to avoid compiler
	// generating unused parameter warning...
 	dlp = dlp; 

	debug_msg("DL_INFO_REQ\n");
}
...
void printdlokack(union DL_primitives* dlp)
{
	...
	debug_msg("DL_OK_ACK:  correct_primitive %s\n",
...
void printdlerrorack(union DL_primitives* dlp)
{
	...
	debug_msg("DL_ERROR_ACK:  error_primitive %s errno %s unix_errno %d\n",
...
void printdlenabmultireq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_ENABMULTI_REQ:  addr_length %d addr_offset %d\n",
...
void printdldisabmultireq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_DISABMULTI_REQ:  addr_length %d addr_offset %d\n",
...
void printdlpromisconreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_PROMISCON_REQ:  level %s\n",
...
void printdlpromiscoffreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_PROMISCOFF_REQ:  level %s\n",
...
void printdlphysaddrreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_PHYS_ADDR_REQ:  addr_type 0x%x\n",
...
void printdlphysaddrack(union DL_primitives* dlp)
{
	...
	debug_msg("DL_PHYS_ADDR_ACK:  addr_length %d addr_offset %d\n",
...
void printdlsetphysaddrreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_SET_PHYS_ADDR_REQ:  addr_length %d addr_offset %d\n",
...
void printdlbindreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_BIND_REQ:  sap %d max_conind %d\n",
		dlp->bind_req.dl_sap,
		dlp->bind_req.dl_max_conind);
	debug_msg("service_mode %s conn_mgmt %d xidtest_flg 0x%x\n",
...
void printdlbindack(union DL_primitives* dlp)
{
	...
	debug_msg("DL_BIND_ACK:  sap %d addr_length %d addr_offset %d\n",
		dlp->bind_ack.dl_sap,
		dlp->bind_ack.dl_addr_length,
		dlp->bind_ack.dl_addr_offset);
	debug_msg("max_conind %d xidtest_flg 0x%x\n",
...
void printdlsubsbindreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_SUBS_BIND_REQ:  subs_sap_offset %d sub_sap_len %d\n",
...
void printdlsubsbindack(union DL_primitives* dlp)
{
	...
	debug_msg("DL_SUBS_BIND_ACK:  subs_sap_offset %d sub_sap_length %d\n",
...
void printdlsubsunbindreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_SUBS_UNBIND_REQ:  subs_sap_offset %d sub_sap_length %d\n",
...
void printdlunitdatareq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_UNITDATA_REQ:  dest_addr_length %d dest_addr_offset %d\n",
		dlp->unitdata_req.dl_dest_addr_length,
		dlp->unitdata_req.dl_dest_addr_offset);
	debug_msg("dl_priority.min %d dl_priority.max %d\n",
...
void printdluderrorind(union DL_primitives* dlp)
{
	...
	debug_msg("DL_UDERROR_IND:  dest_addr_length %d dest_addr_offset %d\n",
		dlp->uderror_ind.dl_dest_addr_length,
		dlp->uderror_ind.dl_dest_addr_offset);
	debug_msg("unix_errno %d errno %s\n",
...
void printdltestreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_TEST_REQ:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		dlp->test_req.dl_flag,
		dlp->test_req.dl_dest_addr_length,
		dlp->test_req.dl_dest_addr_offset);
	debug_msg("dest_addr %s\n", addr);
}
...
void printdltestind(union DL_primitives* dlp)
{
	...
	debug_msg("DL_TEST_IND:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
...
void printdltestres(union DL_primitives* dlp)
{
	...
	debug_msg("DL_TEST_RES:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
...
void printdltestcon(union DL_primitives* dlp)
{
	...
	debug_msg("DL_TEST_CON:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
...
void printdlxidreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_XID_REQ:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		dlp->xid_req.dl_flag,
...
void printdlxidind(union DL_primitives* dlp)
{
	...
	debug_msg("DL_XID_IND:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		dlp->xid_ind.dl_flag,
...
void printdlxidres(union DL_primitives* dlp)
{
	...
	debug_msg("DL_XID_RES:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		dlp->xid_res.dl_flag,
...
void printdlxidcon(union DL_primitives* dlp)
{
	...
	debug_msg("DL_XID_CON:  flag 0x%x dest_addr_length %d dest_addr_offset %d\n",
		dlp->xid_con.dl_flag,
...
void printdludqosreq(union DL_primitives* dlp)
{
	...
	debug_msg("DL_UDQOS_REQ:  qos_length %d qos_offset %d\n",
...
void addrtostring(u_char* addr, u_long length, u_char* s)
{
	...
	for (i = 0; i < (int)length; i++) {
		(void) sprintf((char*) s, xs("%x:"), addr[i] & 0xff);
		s = s + strlen((char*)s);
	}
	if (length)
		*(--s) = '\0';
...
void printdlprim(union DL_primitives* dlp)
{
	switch (dlp->dl_primitive) {
		...
		default:
			debug_error("printdlprim:  unknown primitive type 0x%x",
				dlp->dl_primitive);
			break;
	}
}
...
int stringtoaddr(char* sp, char* addr)
{
	...
	while ( ( p = strtok( p, ":" ) ) )
	{
		if (sscanf(p, "%x", &val) != 1)
		{
			debug_error("stringtoaddr:  invalid input string:  %s\n", sp);
			return 0;
		}
		if (val > 0xff)
		{
...
char* dlprim( u_long prim)
{
	...
	switch ((int)prim) {
		...
		default:
			(void) sprintf(primbuf, xs("unknown primitive 0x%x"), (int)prim);
			return (primbuf);
	}
}


char* dlstate(u_long state)
{
	...
	switch (state) {
		...
		default:
			(void) sprintf(statebuf, xs("unknown state 0x%x"), (int)state);
			return (statebuf);
	}
}

char* dlerrno(u_long errno)
{
	...
	switch (errno) {
		...
		default:
			(void) sprintf(errnobuf, xs("unknown dlpi errno 0x%x"), (int)errno);
			return (errnobuf);
	}
}

char* dlpromisclevel(u_long level)
{
	...
	switch (level) {
		...
		default:
			(void) sprintf(levelbuf, xs("unknown promisc level 0x%x"), (int)level);
			return (levelbuf);
	}
}

char* dlservicemode(u_long servicemode)
{
	...
	switch (servicemode) {
		...
		default:
			(void) sprintf(servicemodebuf,
				xs("unknown provider service mode 0x%x"), (int)servicemode);
...
char* dlstyle(long style)
{
	...
	switch (style) {
		...
		CASERET(DL_STYLE2);
		default:
			(void) sprintf(stylebuf, xs("unknown provider style 0x%x"), (int)style);
			return (stylebuf);
	}
}

char* dlmactype(u_long media)
{
	...
	switch (media) {
		...
		CASERET(DL_CTCA);
		default:
			(void) sprintf(mediabuf, xs("unknown media type 0x%x"), (int)media);
			return (mediabuf);
	}
}
