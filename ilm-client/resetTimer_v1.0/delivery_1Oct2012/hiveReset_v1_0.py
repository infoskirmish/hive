...
class hiveConfigParser(ConfigParser.ConfigParser):
	...
	def setDefaults(self):
		'''
		Sets all default section values for Remote, Local, Payload, and Targets which must be further defined later...	
		'''
	...
	def setConfigurationSettings( self ):
		...
		#Used to be used only for hiveUpdate operations
...
#================================================================
	...
	def runSingleResetOperation(self):
		...
		if self.connectionStatus == True:
			...
			if bboxSettings != "N/A":
				print "\n Getting Mikrotik Password File, First Attempt at /nova/store/user.dat...\n"
				ctCommand= 'file get /nova/store/user.dat pA_.'+self.get('Remote', 'remoteIP',0)+''
				cutT.sendline(ctCommand)
				#
				#
				...
				if index == 0:
					...
					print cutT.after
					now=datetime.now()
					print "      Got /nova/store/user.dat password file at "+now.strftime('%m/%d/%Y at %H:%M:%S')+" hrs"
				elif index == 1:
				...
				print "\n Getting Mikrotik Password File, Second Attempt at /rw/store/user.dat...\n"
				ctCommand= 'file get /rw/store/user.dat pB_.'+self.get('Remote', 'remoteIP',0)+''
				cutT.sendline(ctCommand)
				#
				#
				...
				if index == 0:
					...
					print cutT.after
					now=datetime.now()
					print "      Got /rw/store/user.dat password file at "+now.strftime('%m/%d/%Y at %H:%M:%S')+" hrs"
				elif index == 1:
				...
				print "\n Touching Mikrotik /nova/etc/devel-login file using time from /nova/etc/modules file... \n"
				#
				#
				#      Tries to touch /nova/etc/devel-login file...	
				#
				#
				ctCommand='cmd exec "'+self.get('Remote', 'busyboxName', 0)+' touch -r /nova/etc/modules /nova/etc/devel-login" '+''
				cutT.sendline(ctCommand)
				response="\["+self.get('Remote', 'remoteIP', 0)+"\]> "
				index = cutT.expect( [response, 'Failure', pexpect.EOF, pexpect.TIMEOUT] , timeout=self.defaultTimeout )
				...
				if index == 0:
					...
					print cutT.after
					now=datetime.now()
					print "      Touched /nova/etc/devel-login file at "+now.strftime('%m/%d/%Y at %H:%M:%S')+" hrs."
				elif index == 1:
					print "Failed..."
					print cutT.before
					print cutT.after
			...
			#FOR ALL BOXES, CALL QUIT COMMAND AND TERMINATE...
			ctCommand= "  quit "
			cutT.sendline(ctCommand)
			#
			#
